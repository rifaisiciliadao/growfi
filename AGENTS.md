...
- **Subgraph hooks of note**: `useCampaignInvestors(address)` folds Purchase events per buyer for the InvestorList widget; `useBatchProducerProfiles(addresses[])` resolves wallet → ProducerRegistry profile (name + avatar + social attestation fields, plus legacy `kyced`) in one GraphQL round-trip with a `force-cache` fetch of the off-chain JSON; `useSubgraphProducer(address)` reads `profileURI / version / kyced / kycSetAt` plus social attestation fields for a single producer (used by ProducerCard sidebar + `/grower/[address]` H1/social panel); `useProducerAggregate(address)` powers the producer dashboard KPIs.
