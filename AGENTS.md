...
render social fields unless ProducerRegistry V2 and its second subgraph data
source are live. Mainnet V2 is
`0x267901bB08cb864b204D92185Fac8d6f9dee0F98` from block `25502657`; the
immutable legacy registry at `0x651fb29e69Bde3ADE988e8E75e9A3012272D2de5`
remains indexed for historical profiles but does not expose the social ABI.
Frontend and backend mainnet address resolution map that exact legacy address
to V2 as a deployment-safety guard; other explicit registry overrides remain
supported.
...
a fresh full v4 deploy from the 2026-06 audit-hardened build, with `growfi.dev`,
the frontend env, and the ugraph indexer all pointed at mainnet. Full address
...
The rollout isolates staking rewards by season: only stake opened in the active
season contributes to that season's rate and `totalYieldOwed`; upgraded vaults
were initialized with their exact eligible live stake. Treasury intrinsic-floor
accounting includes both liquid and Treasury-staked CampaignTokens, while
`Factory.unstakeGrowfiTreasury` → `Treasury.unstakeFromCampaign` →
`StakingVault.forceUnstake` returns Treasury positions without a penalty. Sale
activation is explicit but permissionless after `minCap`, so a producer cannot
leave funded escrow locked indefinitely and a purchase still cannot silently
activate/release escrow in the same transaction.
...
- **`ProjectUpdatesModule`** (whitelisted, producer-attaches post-create): producer-only on-chain project update feed. `postProjectUpdate(metadataURI, contentHash)` stores a monotonically increasing update id, author, metadata URI, and keccak256 hash of the off-chain JSON body; `setProjectUpdateHidden(updateId, bool)` hides/unhides bad or obsolete updates without deleting the evidence trail. Metadata lives in DO Spaces via the backend; emoji reactions are off-chain and wallet-signed. Module type: `keccak256("growfi.type.project.updates")`; kind: `keccak256("growfi.project.updates.v1")`; storage slot: `keccak256("growfi.module.project.updates.v1")`.
...
separate `silvi-bridge` app owns the API key and serves `/map/` + `/api/silvi/*`.

Subgraph schema v5.2.0 tracks split/direct issue/project-update state on `Campaign`:
`directIssuedTokens`, `directIssueCount`, `proceedsSplitActive`,
`proceedsSplitPromoter`, `proceedsSplitPromoterBps`,
`proceedsSplitProducerBps`, `projectUpdateCount`,
`visibleProjectUpdateCount`, and derived `directIssues` / `projectUpdates`.
The `DirectIssue` entity is created from `CampaignTokensIssued`; `ProjectUpdate`
is created from `ProjectUpdatePosted` and toggled by `ProjectUpdateHidden`;
...
- **Factory owner / protocol governance is a Safe multisig (assumed honest).** The factory owner is a Gnosis **Safe** controlled by more than one signer. Every `onlyOwner` power — factory upgrade, GROW Treasury custody paths (`rescueToken`, `releaseGrow`, accepted-stablecoin add/remove, `addTrackedCampaign`, `allocateToCampaign`), `setProxyAdminOwner`, emergency `pauseCampaign` — is therefore treated as trusted: the multisig will not act adversarially against the contracts or drain the Treasury. Audit findings that reduce to "a malicious/compromised factory owner could …" are **accepted under this trust model** and intentionally not code-mitigated (no extra timelock added on-chain). This trust does NOT extend to untrusted actors — producers self-dealing and arbitrary buyers/holders are still hardened in code (auto-activation removed, `minCap > 0` enforced for Treasury tracking, etc.).
...
- `Campaign.depositUSDC(seasonId, walletCap)` (v3.4 producer entry) and `Campaign.settleSeasonShortfall(seasonId)` (post-deadline permissionless) both forward to `HarvestManager.depositFromCollateral` which splits **98% → pool, 2% → feeRecipient** on every deposit. Producer sizes the gross via `HarvestManager.remainingDepositGross(seasonId)`. `settleSeasonShortfall` now marks `seasonShortfallSettled` only when the obligation is fully covered; empty collateral reverts `ZeroAmount`, partial collateral remains retryable, and debt restructuring blocks later collateral settlement for that season.
...
- **USDC redeem is a 2-phase flow.** Each phase emits its own event, and the names are semantically correct:
  ...
  - `claimUSDC(seasonId)` — after `claimEnd`, holder pulls their pro-rata USDC. Emits `USDCRedeemed(user, seasonId, amount)` — this is the ONLY event where USDC actually transfers. The post-claim-window gate keeps `usdcOwed` final before any transfer math runs.
...
- **`ProducerRegistry` v4 — social attestation**. The self-served profile surface (`setProfile`, `profileURI`, `version`) and legacy KYC admin path are retained, but the trust badge is now a social attestation, not KYC. Added: `isSocialVerifier[]`, `grantSocialVerifier`, `revokeSocialVerifier`, `socialNonce(producer)`, `claimSocialAttestation(attestation, signature)`, `setSocialAttestation`, `revokeSocialAttestation`, and `hasActiveSocialAttestation`. The backend verifies the public social proof. Active social platforms are only X/Twitter, TikTok, and Website; Instagram and LinkedIn must remain `coming soon` in UI/API until verification can bind the public proof to the selected account author, not just a domain/path and challenge code. On Sepolia (`CHAIN_ID=11155111`), backend EAS relay is enabled by default when `SOCIAL_VERIFIER_PRIVATE_KEY` exists unless `SOCIAL_EAS_ENABLED=false`; all other chains remain explicit opt-in via `SOCIAL_EAS_ENABLED=true`. When EAS is enabled it must create a real EAS attestation, use that non-zero EAS UID in `SocialAttestation.attestationUID`, then relay `ProducerRegistry.setSocialAttestation` if `SOCIAL_REGISTRY_RELAY` is not false. Do not call `SchemaRegistry.getSchemaUID`: EAS SchemaRegistry does not expose that function; compute the UID locally as `keccak256(encodePacked(schema, resolver, revocable))`. Mainnet EAS is `0xA1207F3BBa224E2c9c3c6D5aF63D0eb1582Ce587`, SchemaRegistry is `0xA7b39296258348C78294F95B872b282326A97BDF`, and the GrowFi schema UID is `0x78422879833ca667e9b3ea79d6aaa24328d751493bbd42c92d271d7e94f40caa`. Sepolia EAS `0xC2679fBD37d54388Ce493F1DB75320D236e1815e` returns SchemaRegistry `0x0a7E2Ff54e76B8E6659aedc9103FB21c038050D0` from `getSchemaRegistry()`. The backend verifier address must be granted `isSocialVerifier` before relay works. The EIP-712 claim path remains only as a fallback for environments without backend relay. Frontend uses `<SocialVerificationBadge>` on campaign/grower/investor surfaces and `<SocialVerificationPanel>` inside the self-profile Create/Edit flow on `/grower/[address]` (connected wallet must equal the route address; this is not an owner/admin permission); do not resurrect `<KycBadge>` for the public trust signal. The frontend social path is build-time gated by `NEXT_PUBLIC_ENABLE_SOCIAL_VERIFICATION=true`: enable it only for environments whose ProducerRegistry contract and subgraph schema both expose the social fields/functions. Because Next.js bakes `NEXT_PUBLIC_*` at Docker build time, every production public feature flag and address override must be listed as both `ARG` and `ENV` in `platform/frontend/Dockerfile`; DO App Platform build env alone is not enough.
- **Subgraph schema v5.3.2** carries `expectedAnnualHarvestUsd / expectedAnnualHarvest / firstHarvestYear / coverageHarvests / collateralLocked / collateralDrawn / activeStakerCount` on `Campaign`, `CampaignStaker` aggregates active wallet counts per campaign, legacy `kyced / kycSetAt` on `Producer`, and social attestation fields `socialPlatform / socialHandle / socialProfileUrl / socialProofUrl / socialProofHash / socialAttestationUID / socialIssuedAt / socialExpiresAt / socialVerifiedAt / socialRevokedAt`. Mainnet retains the legacy ProducerRegistry source from block `25328624` and adds ProducerRegistry V2 from block `25502657`; both write the same `Producer` ids. Production `latest` resolves the logical deployment `growfi` to physical storage `growfi@5.3.2`; `5.3.1` remains registered against storage `growfi` as the immediate rollback target. The API must keep `UGRAPH_DEPLOYMENT=growfi`, while the mainnet indexer writes through `UGRAPH_INDEXER_DEPLOYMENT=growfi@5.3.2`. Handlers: `handleCollateralLocked`, `handleCollateralShortfallSettled`, `handleKycSet`, `handleSocialAttestationSet`, `handleSocialAttestationRevoked`, plus staking handlers maintain `Campaign.activeStakerCount` from `Staked` / `Unstaked`. `subgraph.yaml` is Ethereum Mainnet only; `subgraph.sepolia.yaml` is Ethereum Sepolia only.
...
- `script/DeployMainnet.s.sol` — fresh full mainnet deploy: real USDC, Chainlink
  USDC/USD feed, 5 satellite impls, factory proxy, 5 module impls (Sale,
...
| `GrowfiTreasury` | Multi-stable basket (allowlist with per-token Chainlink USD feed + heartbeat + `minPriceBps`/`maxPriceBps` depeg bands) + tracked CampaignTokens. `intrinsicFloorPrice()` returns USD-18 per circulating GROW; **state-aware** — only Active campaign CTs count, including active Treasury-owned staking positions, while depegged stables are silently excluded. Manual `allocateToCampaign` + auto-fired `allocateAcrossTracked` (callable by factory OR by Token via the buy hook, gated by `automationEnabled`). `releaseGrow(to, amount)` is the ONLY exit for the team-reserve GROW the Treasury holds (rescue + redeem both forbid GROW). `claimUsdcAndDistribute` is permissionless and splits 80% to StakingPool / 20% retained per `stakerRewardBps`. |
...
totalValue = Σ_stable (balance × scale × livePriceUsd / 1e18)   [excluding depegged]
           + Σ_camp   ((treasury CT balance + active treasury staked CT) × pricePerToken / 1e18)
                                                                  [Active state only]
...
Tier thresholds are snapshotted on the first non-excluded buy per campaign, so later producer cap edits cannot reopen cheaper GROW tiers. SaleClassic calls the GROW minter hooks with `try/catch`; a broken minter must not brick buys, soft-cap activation, or failed-campaign buyback.
...
**Admin UI separation**:
- The public Next.js frontend must not expose protocol admin pages. `/grow/admin`
  was removed; do not reintroduce admin controls under `growfi.dev`.
- `platform/admin/` is the separate admin app for `admin.growfi.dev`. It is a
  Vite React app with wallet-only login, no backend admin key and no private-key
  custody. Current scope is intentionally minimal: read the Ethereum mainnet
  FeeSplitter USDC balance, preview the split, and submit
  `GrowfiFeeSplitter.flushToken(USDC)` from an allowlisted admin wallet.
- `flushToken` is permissionless on-chain; the wallet allowlist is a UI gate so
  operators do not accidentally expose controls to the wrong account. Default
  allowlist includes the operations Safe, the two Safe owners, and the current
  factory owner.
- **Hide/Unhide is UI-only.** `factory.setCampaignHidden(addr, bool)` (onlyOwner)
  flips the `hiddenCampaigns` mapping and emits `CampaignHiddenSet`. The subgraph
  mirrors it on `Campaign.hidden`. The frontend `useSubgraphCampaigns()` hook
  filters `where: { hidden: false }` by default; only explicit internal/admin
  tooling and `EscrowClaimPanel` (so users with pending escrow on a hidden
  campaign can still claim) should opt back in. `useSubgraphCampaign(id)`
  (single-row) and `findCampaignByName` are NEVER filtered — direct-link access
  to a hidden campaign still works (the on-chain factory stays permissionless),
  and the name-uniqueness pre-flight on `/create` correctly treats hidden
  campaigns as still occupying the namespace. `useProducerCampaigns` filters
  hidden out, so a hidden campaign disappears from the public grower profile
  (the producer can still reach it via direct URL).
...
- Versioned per redeploy behind ugraph. Current live endpoint is `https://ugraph.growfi.dev/subgraphs/growfi/latest/gn`; direct legacy endpoints and the stale `prod` tag were removed during cleanup. Do not recreate a `prod` alias without explicit user request.

## Deployments

See `CONTRACTS.md` for current Ethereum Mainnet addresses (factory proxy, impls, registries, GROW contracts, frontend env vars). See `DEPLOY.md` for the DigitalOcean App Platform runbook (frontend + backend) — `.do/app.yaml` spec, per-service Dockerfiles, CLI-only secret injection via `doctl apps update` + `jq` spec patching, and the auto-deploy-on-push flow against `main`. Live at `https://growfi.dev` (custom domain, registered in Reown WalletConnect's allowlist for the project ID baked into `.do/app.yaml`). The bare DO subdomain `https://growfi-test-m9s8u.ondigitalocean.app` still resolves but is no longer the canonical URL — surface `growfi.dev` everywhere user-facing. Production is app `growfi-mainnet`, id `9e4019f4-8dbc-4170-8546-ce7d8579e3a4`, under the DigitalOcean RIFAI team selector `i=b9d43f`. The canonical `doctl` context name is `turinglabs`; keep it authenticated for that team. The separately saved `default` context belongs to another team and cannot access this app. The separate admin app uses `.do/admin.yaml` and should be published at `https://admin.growfi.dev`; do not route it through the public frontend component. **Path-based ingress gotcha**: the backend rule needs `preserve_path_prefix: true` — by default DO App Platform strips the match prefix before forwarding, so a rule matching `/api` would deliver `/upload` to the backend while our routes are `/api/upload` → every endpoint 404s. The flag is documented inline in `.do/app.yaml`.
...
├── subgraph/   Subgraph indexer served through ugraph
...
- **Chains**: Ethereum Mainnet is the production default (`NEXT_PUBLIC_CHAIN_ID=1`). Ethereum Sepolia (11155111), Base Sepolia (84532), and Base Mainnet (8453) remain wired for archived/reference deployments. Chain set is selected by `NEXT_PUBLIC_CHAIN_ID`. RPC fallback for mainnet uses `NEXT_PUBLIC_MAINNET_RPC_URL` when present, then public RPC fallbacks; Sepolia uses `NEXT_PUBLIC_SEPOLIA_RPC_URL` then publicnode/rpc.sepolia.org/1rpc. Block explorer URL builder in `src/lib/explorer.ts` maps the chain id to etherscan/basescan correctly — don't hardcode `sepolia.basescan.org` anywhere; always go through `txUrl(hash)` / `addressUrl(addr)`. Wrong-network UX is centralized in `src/lib/chains.ts`, `src/lib/useExpectedChain.ts`, and `components/NetworkGuard.tsx`; compare the connected wallet chain to `NEXT_PUBLIC_CHAIN_ID`, prompt switch/add-chain when supported by the wallet, and gate write surfaces until the expected chain is active.
- **i18n**: EN / IT / ES / FR / PT. Provider in `src/i18n/LocaleProvider.tsx` — browser auto-detect + `localStorage["growfi:locale"]` persistence. Messages in `src/messages/<locale>.json`.
- **Analytics**: `app/layout.tsx` injects Google Analytics via `next/script` with measurement id `G-PN63LEDTBB`. Keep it in the root layout so every public route is tracked after hydration; do not paste raw `<script>` tags into individual pages.
...
- **Pages**:
  - `/` — **Landing page** (cinematic marketing surface, not discovery). Fullscreen looping video background + 6 sections, in order: Hero → Campaigns → HowItWorks → dark Trust → light Partners → LandingFooter. Campaigns is deliberately ABOVE HowItWorks so the live proof (real campaigns from the subgraph) lands before the longer "how it works" explainer. The landing pitch is narrowed to **syntropic agroforestry** (not Sicily-specific, not commodity-specific): the rotating hero highlight cycles agroforestry-compatible commodity→product pairs (olives/oil, cacao/chocolate, shade coffee, grapes/wine, orchard beehives/honey). Hard rule: no invented figures ("$1,000 → 5L oil") — live numbers come from the subgraph, pattern copy replaces specific math. The landing hero badge now advertises the open seed round and links to `/investors`; keep it fundraising-oriented, not "first campaign live". The landing `<Nav>` mirrors the internal `<Header>` compact desktop pattern: `Explore`, `Investors`, and `$GROW` are absolutely centered on `lg+` so wallet width cannot visually skew them; on `md` desktop/tablet widths they move into the far-right hamburger with `Feed`, `Portfolio`, and `Create/Richiedi invito`; mobile keeps the full hamburger sheet. The `<Header>` itself has no chain-name pill — the wrong-network fallback pill is still there, but we don't surface "Base Sepolia" in happy-path chrome. Fonts wired via `next/font/google` into three CSS vars scoped by `.landing-root`: Be Vietnam Pro (`--font-header`, 700), Crimson Text (`--font-body`, 400 + italic), Plus Jakarta Sans (`--font-accent`, 700/800) — `--font-accent` is used only by `RotatingHighlight` for a distinctive display cut without touching global headlines. Animations (`reveal`, `fade-rise`, `slow-pan`, `live-ring`, `shimmer`, etc.) + glass-blur classes (`.glass-section`, `.glass-section-dark`, `.glass-card`) live in `globals.css` under the `.landing-root` scope so they don't leak into the rest of the app. `.glass-section` background is `rgba(255,255,255,0.84)` (was 0.58) so text stays readable over the looping video — don't drop below ~0.80 or legibility regresses. Components under `src/components/landing/*` + hooks under `src/lib/landing/*`. On `/`, the global `<Header>` + `<Footer>` are hidden by `ConditionalChrome` (`usePathname() === "/"`) — the landing has its own `<Nav>` and `<LandingFooter>`; every other route keeps the wallet-aware chrome.
    The route exports `dynamic = "force-dynamic"` because the landing loads live subgraph-backed campaign data client-side and must not be cached by Cloudflare/DO as a year-long static HTML shell with stale JS chunks.
    ...
    - **Campaigns section** renders `LiveCampaignCard` for every on-chain campaign (via `useSubgraphCampaigns` + `useCampaignMetadata` for the image/product/location), then appends a single `CreateCampaignCard` dashed-green CTA at the end → `/create`. No hardcoded placeholder cards. The section heading + intro are written to be producer-invitational ("Una campagna, un raccolto reale. · Finanzi oggi, ritiri al raccolto.") and describe the USDC→$CAMPAIGN→$YIELD→redeem cycle without name-dropping any specific live campaign (the live one renders as a normal card like any other).
      `CAMPAIGN_FIELDS` in `src/lib/subgraph.ts` must stay limited to fields required by discovery/detail surfaces and present on all currently live subgraphs. Feature-specific additive fields, such as project-update counters, belong in their own queries so a stale testnet/mainnet subgraph cannot blank the campaign list.
  ...
  - `/campaign/[address]` — dynamic tabs. The active tab mirrors the `?tab=` query param (`invest` | `shop` | `updates` | `silvi` | `stake` | `harvest` | `info` | `manage`), so deep links like `ProducerAggregateDashboard`'s `?tab=manage` land directly on the right panel. `shop` is visible only when the Ecommerce module is attached, `silvi` only when campaign metadata has a Silvi Protocol link, and `manage` only for the producer. Invalid or temporarily unavailable tab params fall back to `invest`; keep this guard or direct links like `?tab=shop` can leave the left column blank while module/subgraph data is still resolving. Local state stays in sync via a `useSearchParams` effect.
    ...
    - **Shop** — `EcommerceShopPanel` renders when the Ecommerce module is attached. It reads `EcommerceStore`, SKU rows, and catalog JSON from the subgraph/backend static storage, then runs the checkout flow: private order draft → on-chain `buySku` → purchase receipt email. On desktop the product list and checkout summary use a two-column grid with `items-start` / `content-start`; without those constraints the product card can stretch to the height of the checkout card and make screenshots look empty or broken. Missing SKU config must render as "SKU not configured on-chain" rather than "Sold out"; sold out is only for an existing active SKU with zero inventory.
    - **Shop manager** — `EcommerceModuleManager` hydrates the form from the current catalog URI and the selected on-chain SKU. Legacy catalogs that only have a bytes32 `skuId` keep that id internally (`skuIdOverride`) while showing a readable SKU key and preserved-id hint, so updates do not accidentally hash a different SKU; new catalog uploads also include `skuKey` for future readability. Product images can be uploaded via `/api/upload` before `/api/ecommerce/catalog`. On an already-attached Ecommerce module, publishing writes `setSku` before `setCatalogURI` and skips `setRepaymentAllocationBps` unless the Repayment module is active and the allocation actually changed, so a stalled optional repayment update cannot leave the public shop saying the SKU is missing. Producer-entered repayment allocation is a percentage and is converted to bps only when the Repayment module is active; if Repayment is absent/disabled, the field is disabled and writes `0` so `buySku` cannot later revert with `RepaymentModuleInactive`. Protocol fee is displayed read-only and is not updated by the manager UI.
    - **Staking** — `StakingPanel` reads positions via `getPositions` + batched `positions`/`earned` (10s refetch), guards against `NoActiveSeason` (pre-flight check on `seasons[currentSeasonId].active`) and `TooManyPositions` (50/user cap). Live unstake-penalty %. Uses dynamic `yieldToken.symbol()` everywhere. The tab also renders active campaign stakers from `useCampaignStakers(campaignAddress)`, resolving ProducerRegistry profiles, social badge state, and ENS names before falling back to the shortened wallet address.
  ...
  - `/grower/[address]` — public profile page. **The route was `/producer/[address]` until 2026-05-06**; renamed to `/grower/[address]` along with all visible copy (EN: grower, IT: coltivatore, ES: cultivador, FR: cultivateur) because "producer" was scoring badly in user testing — felt corporate/extractive vs the hands-in-the-soil framing the protocol is going for. **Internal symbols kept the old name** (component files like `ProducerAggregateDashboard.tsx`, hooks like `useSubgraphProducer`, types like `SubgraphProducer`, and the on-chain `ProducerRegistry` contract). They don't surface to users and renaming them is large-blast-radius churn — accept the asymmetry. **The i18n namespace was renamed `producer.*` → `grower.*`** in all 4 messages files (the bulk regex on the JSON also touched the keys, which initially broke the page rendering raw key names; we kept the rename and updated all `useTranslations("producer")` / `useTranslations("producer.form")` / `t("producer")` call sites to `grower` instead of reverting). If you add a new key inside this surface, use `grower.*` not `producer.*`. The page hierarchy is cover banner (always rendered, with an organic placeholder) → overlapping avatar and public identity → bio/profile controls → grower-only aggregate dashboard → campaign cards → wallet exposure. The self-profile edit trigger is a 44px accessible pencil control in the banner's top-right corner. When the connected wallet IS the grower, `ProducerAggregateDashboard` shows 4 KPIs (campaigns, total raised, USDC still owed, pending reports) + deep-link chips to the specific campaigns that need action (`?tab=manage` on each). Edit form flow: upload via backend → `uploadProducerProfile` → `ProducerRegistry.setProfile` → poll subgraph until `version` bumps, then close and invalidate the parent query. Social verification is part of the same self-profile Create/Edit flow: request a campaign-bound challenge, publish its code on the supported public profile/post, paste the proof URL, sign the canonical challenge with the connected wallet, and let the backend publish EAS plus the V2 registry attestation. The mainnet UI and sponsored relay have been live since 2026-07-10, after ProducerRegistry V2, the EAS schema, verifier wallet, and second subgraph data source were verified.
  - `/investors` — public fundraising page for the GrowFi seed round. It uses the same light app-card visual system as the catalog/FAQ pages, not the homepage photo hero. Copy is chain-aware from `NEXT_PUBLIC_CHAIN_ID`: local Sepolia builds can explain testnet rehearsal, while production mainnet copy stays Ethereum-mainnet-first (why L1: long-term security, finality, native liquidity, diligence credibility). Includes a `$GROW` flywheel section with stacked right-side cards, links the pitch deck at `public/growfi-seed-deck.pdf`, and posts the demo/request form through `src/lib/api.ts` → `POST /api/investors/request`. Localized under `investors.*` in all 5 message files. The global Header/Footer and landing Nav/Footer all expose an Investors link.
...
- **Key components**: `Spinner` (inline spinner for tx states), `CampaignImage` (branded green-gradient fallback with GrowFi logo when no cover image), `Header` (single connected-wallet pill: address-derived initials circle + truncated `0xABCD…1234`, click → `/grower/<address>`. **No more RainbowKit `openAccountModal()` popup** — the Copy/Disconnect modal was useless and the auto-generated emoji avatar (panda, cat, mushroom, etc.) was off-brand. Disconnect lives on the grower profile page as a small text-link, only visible when the connected wallet equals the route address. The `ConnectButton.Custom` render-prop now only consumes `openConnectModal` (initial connect) and `openChainModal` (wrong-network fallback). Desktop nav is intentionally compact: primary links are viewport-centered with absolute positioning on `lg+`, secondary routes sit behind the rightmost icon hamburger with outside-click/ESC close, and `md` widths put all links in that hamburger to prevent overlap; wallet connect sits immediately to its left. **The landing's separate `<Nav>` (`components/landing/Nav.tsx`) mirrors the same pattern** — same pill-as-Link, no account modal — so users get one consistent wallet UI across marketing and app), `LanguageSwitcher`, `Toast` / `ToastProvider` (bottom-right, auto-dismiss, `useTxNotify()` helper wraps push + silences user-rejections + injects explorer links), `InvestorList`, `YieldRateCurve`, `RefreshButton`.
- **Subgraph hooks of note**: `useCampaignInvestors(address)` folds Purchase events per buyer for the InvestorList widget; `useCampaignStakers(address)` reads indexed `CampaignStaker` rows for the Staking tab's active-staker list; `useBatchProducerProfiles(addresses[])` resolves wallet → ProducerRegistry profile (name + avatar + social attestation fields, plus legacy `kyced`) in one GraphQL round-trip with a `force-cache` fetch of the off-chain JSON; `useSubgraphProducer(address)` reads `profileURI / version / kyced / kycSetAt` plus social attestation fields for a single producer (used by ProducerCard sidebar + `/grower/[address]` H1/social panel); `useProducerAggregate(address)` powers the producer dashboard KPIs.
- **ENS reverse-lookup** (`platform/frontend/src/lib/ens.ts`): standalone viem `publicClient` on Ethereum mainnet (NOT in the wagmi config — wagmi active chains are selected by `NEXT_PUBLIC_CHAIN_ID`). Two hooks: `useEnsName(address)` and `useBatchEnsNames(addresses[])`, both cached 1h via React Query. RPC fallback chain is public (eth.llamarpc, cloudflare-eth, ankr) with a `NEXT_PUBLIC_MAINNET_RPC_URL` override for an Alchemy/Infura key under demo load. **Display-name priority chain everywhere**: internal ProducerRegistry profile → ENS reverse name → `t("producer.anonymous")` ("Anon grower") fallback. Internal profile is the strongest claim (gated by the producer's own signature); ENS is a strong public identity but NOT domain-specific. Social verification is NOT promoted by ENS — only an active ProducerRegistry social attestation triggers `<SocialVerificationBadge>`. The fallback says "Anon grower" rather than "Anonymous producer" because these wallets are buyers/stakers, not necessarily campaign producers.
- **Known tokens catalog**: `src/contracts/tokens.ts` — curated `KNOWN_TOKENS` list with per-chain addresses and Chainlink feeds. On mainnet, USDC is `enabled: true` and wired to the real USDC token + Chainlink USDC/USD feed; USDT/DAI/WETH mainnet addresses are present but disabled until factory/Treasury policy explicitly allows them. Testnet `mUSDC` stays enabled only on Sepolia-style demo chains. Used by `/create` step 3, `ProducerManagePanel` → AcceptedTokensManager. **The `mUSDC` Sepolia address is sourced from `process.env.NEXT_PUBLIC_USDC_ADDRESS`** (same env that drives `getAddresses().usdc` and `factory.usdc`) so the catalog stays in sync with the live factory across redeploys. Don't hardcode the mUSDC address back into the catalog: the previous hardcoded mock drifted from `factory.usdc` after a redeploy and the BuyPanel "Mint 10,000 mUSDC" button silently disappeared, because `selected.address` (from the campaign's accepted-tokens) no longer matched `mockUsdcAddress` (from `getAddresses().usdc`) and the gating condition collapsed.
- **Env** (`.env.local`): `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID`, `NEXT_PUBLIC_BACKEND_URL`, `NEXT_PUBLIC_CHAIN_ID`, `NEXT_PUBLIC_FACTORY_ADDRESS`, `NEXT_PUBLIC_USDC_ADDRESS`, `NEXT_PUBLIC_SUBGRAPH_URL`, optional `NEXT_PUBLIC_SILVI_MAP_BASE_URL` for the Silvi bridge, plus the current registries, extra stables, module impls, and GROW addresses (`NEXT_PUBLIC_REGISTRY_ADDRESS`, `NEXT_PUBLIC_PRODUCER_REGISTRY_ADDRESS`, `NEXT_PUBLIC_USDT_ADDRESS`, `NEXT_PUBLIC_DAI_ADDRESS`, `NEXT_PUBLIC_REPAYMENT_IMPL`, `NEXT_PUBLIC_ECOMMERCE_IMPL`, `NEXT_PUBLIC_DEBT_RESTRUCTURING_IMPL`, `NEXT_PUBLIC_PROCEEDS_SPLIT_IMPL`, `NEXT_PUBLIC_DIRECT_ISSUE_IMPL`, `NEXT_PUBLIC_PROJECT_UPDATES_IMPL`, `NEXT_PUBLIC_GROW_*`). Keep these aligned with the live mainnet deploy and DigitalOcean build-time env.
...
- **Prod build**: `next.config.ts` ships with `output: "standalone"` so the Docker image copies only `.next/standalone/server.js` plus static assets (~150 MB), not the full `node_modules`. The multi-stage `platform/frontend/Dockerfile` accepts every `NEXT_PUBLIC_*` as a build ARG — miss one and the build fails loudly. DigitalOcean App Platform receives the current Ethereum Mainnet addresses at BUILD_TIME scope through its app spec/env; secret public vars (only `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID`) live in the dashboard. Root `DEPLOY.md` is the runbook.
...
| `POST /api/project-updates/metadata` | Producer-side project update metadata upload. Body `{ campaign, title, body, imageUrl? }`; validates the campaign address and rich-text body, writes public JSON under `project-updates/<campaign>/...`, and returns `{ key, url, contentHash, metadata }`. The returned `url` + `contentHash` are passed to `ProjectUpdatesModule.postProjectUpdate`. |
| `GET /api/project-updates/:campaign/:updateId/reactions?address=` | Reads the off-chain emoji reaction summary for one on-chain project update. Returns `{ counts, total, viewerEmoji, updatedAt }`; `viewerEmoji` is included only for the optional address query. |
| `PUT /api/project-updates/:campaign/:updateId/reactions/me` | Wallet-signed reaction writer. Body `{ campaign, updateId, address, emoji, issuedAt, nonce, signature }`; verifies the EIP-191 signature against `buildProjectUpdateReactionMessage`, stores one emoji per wallet, and treats empty `emoji` as removal. |
...
| `POST /api/merkle/generate` | Body: `{ campaign, seasonId, totalProductUnits, totalYieldSupply, holders: [{ user, yieldAmount }], minProductClaim?, authorization: { expiresAt, signature } }`. The backend resolves `campaign.producer()`, verifies the producer's short-lived EIP-191 publication signature, and builds OZ-compatible sorted-pair leaves. It allocates against `totalYieldSupply`, rejects over-allocation, and writes once with `If-None-Match: *` to immutable `merkle/<campaign>/<seasonId>/<root>.json`; callers cannot overwrite an existing proof set. Returns `{ root, url, count, totalYieldSupply }` for `HarvestManager.reportHarvest(..., expectedTotalYieldSupply)`. |
| `GET /api/merkle/:campaign/:seasonId/:root/:user` | Reads the root-addressed tree, verifies the stored root equals the requested on-chain root, and returns `{ user, yieldAmount, productAmount, proof }` for an eligible user. A read-only fallback supports legacy season JSON, but only when its embedded root matches. |
| `POST /api/social-verification/challenge` | Body `{ wallet, campaign?, platform, handle?, profileUrl? }`. Requires `SOCIAL_CHALLENGE_SECRET`; the v2 challenge binds wallet, campaign, chain, registry, profile, nonce, and expiry into the message the wallet must sign. When sponsored EAS relay is active, the campaign is mandatory and its on-chain producer must equal the wallet. |
| `POST /api/social-verification/verify` | Body includes the complete challenge, `proofUrl`, `walletSignature`, and `onchainNonce`. It verifies the wallet's EIP-191 signature before any sponsored transaction, enforces public-only HTTP(S) resolution with DNS pinning (no private/transition addresses, redirects, oversized bodies, or DNS rebinding), checks the public proof text, and atomically reserves the wallet/nonce in private Spaces storage to prevent replay. With `SOCIAL_EAS_ENABLED=true`, it publishes EAS and optionally relays `ProducerRegistry.setSocialAttestation`; failed pre-submission attempts release the reservation, while submitted EAS transactions keep it consumed. |
...
- **Invite admin** — invite approval/rejection is handled through
  `platform/backend/scripts/invite-cli.ts` or direct backend admin API calls with
  `ADMIN_API_KEY`. It is no longer part of `platform/admin/`.
...
- **Frontend** — `<NotificationsSection address={producerAddress}>` mounts on `/grower/[address]` only when the connected wallet equals the route address. Toggle + email input + `useSignMessage()` from wagmi → PUT. Polls status via React Query (`["notifications", "status", lower]`, 30s staleTime). Localized EN/IT/ES/FR under `producer.notifications`.
...
Live through ugraph — **project: growfi · chain: mainnet**. Current endpoint: `https://ugraph.growfi.dev/subgraphs/growfi/latest/gn` (tracks the Ethereum Mainnet v4 module architecture, GROW system, Repayment module, Ecommerce module, and DebtRestructuring module). Direct legacy endpoints and the stale `prod` tag were removed.

Indexed contracts (see `CONTRACTS.md` for authoritative deploy refs):
- `CampaignFactory` @ `0x81c2ecb09B8062cC9F3A4F8682318456304f4aE2` from block `25328624`
- `CampaignRegistry` @ `0xA3AEb95Ff4555E266aa1366000204a75FaD4142B` from block `25328624`
- Legacy `ProducerRegistry` @ `0x651fb29e69Bde3ADE988e8E75e9A3012272D2de5` from block `25328624` and current `ProducerRegistryV2` @ `0x267901bB08cb864b204D92185Fac8d6f9dee0F98` from block `25502657`; Sepolia testnet uses the dedicated `subgraph.sepolia.yaml` registry address.
- GROW system static data sources start at block `25328624`; see `platform/subgraph/subgraph.yaml` for the full list.

Endpoints:
- **Canonical live endpoint**: `https://ugraph.growfi.dev/subgraphs/growfi/latest/gn`
- **Do not use** direct legacy endpoints or a `prod` tag; they were removed.

Frontend client: `platform/frontend/src/lib/subgraph.ts` — minimal fetch+React Query wrapper exposing `useSubgraphCampaigns`, `useSubgraphCampaign(id)`, `useSubgraphMeta()`, and `useFeed()`. Campaign discovery renders `Campaign.totalStaked` as a token amount for active campaign cards; do not label it as a wallet/staker count. The subgraph also indexes `Campaign.activeStakerCount` for surfaces that need active wallet counts. `useFeed()` merges funding `Purchase` events with staking `Position` events, Ecommerce `EcommerceOrder` events, project updates, claims, and sellback activity; shop purchases render as `FeedItem.kind = "shop"` using `grossPaid` as USDC-6 and the localized `activity.verbs.shopBought` label. Off-chain metadata JSON (pointed to by `Campaign.metadataURI`) is fetched by `platform/frontend/src/lib/metadata.ts::useResolvedCampaignMetadata(campaign)` — it first uses subgraph metadata and falls back to reading `CampaignRegistry.metadataOf/versionOf` on-chain when the subgraph is briefly behind; queryKey includes `version` so an on-chain URI rotation invalidates cleanly.

Schema + handlers:
- `schema.graphql` indexes campaign core entities plus v4 module entities: `Module`, `RepaymentPool`, `Repayment`, `EcommerceStore`, `EcommerceSku`, and `EcommerceOrder`. The `Campaign` entity carries `metadataURI` + `metadataVersion` populated by the `CampaignRegistry` handler, a `hidden: Boolean!` flag (default false) flipped by `handleCampaignHiddenSet`, derived module relations, and derived Ecommerce/Repayment relations. The `Producer` entity (id = producer address) carries `profileURI` + `version` populated by the `ProducerRegistry` handler, legacy `kyced / kycSetAt`, and social attestation fields populated/revoked by `handleSocialAttestationSet` and `handleSocialAttestationRevoked`; it has no derived `campaigns` relation — frontend queries campaigns separately with `where: { producer: $addr }`. `Purchase.fundingFee` is populated via the aux `FundingFeeByTx` entity (keyed on tx hash) written by `handleFundingFeeCollected` — which fires BEFORE `handleTokensPurchased` in the v2 `buy()` tx. Pre-v2 purchases have no matching aux row → `fundingFee = 0` (historically correct). **Single-buy-per-tx assumption**: if the UI ever batches via multicall, the tx-hash-keyed aux entity collides and needs re-keying.
...
- **Factory is live** on Ethereum Mainnet at `0x81c2ecb09B8062cC9F3A4F8682318456304f4aE2` (v4 audit-hardened launch, see `CONTRACTS.md` for the full address set). Discovery reads exclusively from the ugraph subgraph — no more mock fallback. Empty subgraph → empty state CTA.
- **Imperative tx flow everywhere** — never use `useWaitForTransactionReceipt` + useEffect for progress state (race-prone: a receipt from the previous tx can briefly match while a new tx is in flight and show wrong success). Always use `waitForTx(hash)` from `@/lib/waitForTx` inside the async handler — it wraps `waitForTransactionReceipt` with `confirmations: 2`, `timeout: 180s`, and a `minVisibleMs` floor (default 1200ms) so the "confirming on-chain…" state is visible even when the receipt is cached. Silence `user rejected/denied` errors; surface everything else. The shared `config` is exported from `src/app/providers.tsx`.
- **RPC fallback** — `providers.tsx` uses a viem `fallback` transport for the active chain selected by `NEXT_PUBLIC_CHAIN_ID`. Current target is Ethereum Mainnet (`1`) with `NEXT_PUBLIC_MAINNET_RPC_URL` optional, then public RPC fallbacks (each with 3 retries @ 500ms, 10s timeout). Sepolia/Base transports remain wired only for archived/reference builds.
...
- **Chain name ↔ id alignment**: frontend uses `NEXT_PUBLIC_CHAIN_ID=1`, subgraph uses `network: mainnet`. Both point at Ethereum Mainnet — keep in sync if you ever target a different network.
- **Subgraph `startBlock`** — current factory/registries and GROW data sources start at `25328624`. Never set to 0 on mainnet/testnet; scanning from genesis wastes hours.
