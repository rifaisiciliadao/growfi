...
| `POST /api/social-verification/challenge` | Body `{ wallet, platform, handle?, profileUrl? }`. Requires `SOCIAL_CHALLENGE_SECRET`; returns a signed short-lived challenge and message `GrowFi social verification: GROWFI-SOCIAL-<wallet>-<nonce>` for the user to publish publicly. |
| `POST /api/social-verification/verify` | Body includes the challenge payload, `proofUrl`, and `onchainNonce`. Fetches the proof URL, requires the page/post text to contain the GrowFi code (or wallet + nonce), hashes the proof, and, when `SOCIAL_EAS_ENABLED=true`, creates a real EAS attestation and returns `eas.attestationUID` plus tx hashes. If `SOCIAL_REGISTRY_RELAY` is true, the backend also writes `ProducerRegistry.setSocialAttestation`; otherwise it returns EIP-712 typed data plus a backend verifier signature for the wallet-claim fallback. |
