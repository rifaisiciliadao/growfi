...
- **v3 productive-asset fields + collateral mechanic** (live on Base Sepolia 2026-04-28). `Campaign` gains three immutable fields set at creation — `expectedYearlyReturnBps` (e.g. 1000 = 10%/yr), `expectedFirstYearHarvest` (product units 1e18), `coverageHarvests` (number of upcoming harvests the producer pre-funds) — plus mutable collateral state (`collateralLocked`, `collateralDrawn`, `seasonShortfallSettled`). Producer calls `lockCollateral(amount)` in USDC during Funding/Active (additive, **no early withdrawal**). After each covered season's `usdcDeadline` lapses, anyone can call `settleSeasonShortfall(seasonId)` once: it reads `HarvestManager.remainingDepositGross`, draws from collateral up to that gap, and forwards via the new `HarvestManager.depositFromCollateral(seasonId, amount)` (only callable by the owning Campaign — wired through `HarvestManager.setCampaign(campaign)` post-deploy). Residual collateral is one-way locked by design (does NOT return to producer). Distribution to holders is a v3.1 TODO. The full math + attack-surface notes live in `docs/05 - Math & Formulas.md §15`.
...
- **`Campaign.initializeV3(yearlyReturnBps, firstYearHarvest, coverageHarvests, usdc)` reinitializer** seeds the three v3 immutable fields + the cached USDC reference on pre-v3 proxies during upgrade. Required for any older campaign that wants to opt into the collateral mechanic. Fresh v3 campaigns skip it because the regular `initialize` writes those slots.
...
- **Subgraph schema v2.3.0** appends `expectedYearlyReturnBps / expectedFirstYearHarvest / coverageHarvests / collateralLocked / collateralDrawn` to `Campaign` and `kyced / kycSetAt` to `Producer`. New handlers: `handleCollateralLocked`, `handleCollateralShortfallSettled`, `handleKycSet`. `CampaignCreated` event signature extended with the three commitment fields — `subgraph.yaml` must use the new signature or the indexer rejects it at validate time.
