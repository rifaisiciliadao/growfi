...
- **`Campaign.initializeV3(annualHarvestUsd, firstHarvestYear, coverageHarvests, usdc)` reinitializer** seeds the v3.2 immutable fields + the cached USDC reference on pre-v3 proxies. We don't actually run any upgrade — every redeploy is a fresh deploy — but the reinitializer stays for future flexibility.
...
- **Subgraph schema v2.5.0** carries `expectedAnnualHarvestUsd / firstHarvestYear / coverageHarvests / collateralLocked / collateralDrawn` on `Campaign` and `kyced / kycSetAt` on `Producer`. Handlers: `handleCollateralLocked`, `handleCollateralShortfallSettled`, `handleKycSet`. `CampaignCreated` event signature carries 16 args (4 addresses + 10 uint256 + 2 indexed addresses); `subgraph.yaml` must match exactly or the indexer rejects at validate time. The earlier 18-arg shape (v2.4.0) was a transitional v3.1 schema.
...
Live on Goldsky — **team: turinglabs · project: growfi · chain: base-sepolia**. Current version: `growfi/2.5.0`, tagged as `prod` (deployed 2026-04-28; tracks the v3.2 redeploy with `expectedAnnualHarvestUsd` + `firstHarvestYear`).

Indexed contracts (see `CONTRACTS.md` for authoritative deploy refs):
- `CampaignFactory` @ `0x0D8be3Da6232a51Ea9218eF47c6Ab085B8940684` from block `40801000`
- `CampaignRegistry` @ `0xc3B052EA719b8BAe6AFb32bfe6b8D2B8fc2580D6` from block `40801014`
- `ProducerRegistry` @ `0xf4c03758F2912EB6D6777cADff678CEa0FDD63Fb` from block `40801019`
- Earlier factories `0xDE26…bF9f` (v3.1), `0x91fD…6BDD` (v3.0), and `0x5178…FF64` (pre-v3) are all abandoned; campaigns deployed there are not migrated. Single seeded test campaign at `0x8714B91da03c8Df7C1cC26464B306d3ee6286F7B` (350k OLIVE × $0.144 = $50,400 max raise, $5,000/yr commitment from year 2030, $15k collateral pre-funding 3 harvests = 2030–2032).
...
- **Factory is live** on Base Sepolia at `0x0D8be3Da6232a51Ea9218eF47c6Ab085B8940684` (v3.2, see `CONTRACTS.md` for the full address set). Discovery reads exclusively from the Goldsky subgraph — no more mock fallback. Empty subgraph → empty state CTA.
