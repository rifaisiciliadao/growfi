...
- **Pages**:
  - `/` — **Landing page** (cinematic marketing surface, not discovery). Fullscreen looping video background + 5 sections: Hero, HowItWorks, Campaigns preview (hardcoded — Sharewood live + 5 `state: "coming"` placeholders; the real subgraph grid lives on separate discovery routes going forward), dark Trust section, Footer. Uses Be Vietnam Pro (headlines) + Crimson Text (body) via `next/font/google` wired into the `--font-header` / `--font-body` vars that `.landing-root` scopes. Animations (`reveal`, `fade-rise`, `slow-pan`, `live-ring`, `shimmer`, etc.) + glass-blur classes (`.glass-section`, `.glass-section-dark`, `.glass-card`) live in `globals.css` under the `.landing-root` scope so they don't leak into the rest of the app. Components under `src/components/landing/*` + hooks under `src/lib/landing/*`. On `/`, the global `<Header>` + `<Footer>` are hidden by `ConditionalChrome` (`usePathname() === "/"`) — the landing has its own `<Nav>` and `<LandingFooter>`; every other route keeps the wallet-aware chrome.
