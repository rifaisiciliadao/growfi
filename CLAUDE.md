...
- **Pages**:
  ...
  - `/campaign/[address]` — Invest / Stake / Harvest / Info tabs. Hero (title, image, location, breadcrumb) resolves from `useSubgraphCampaign` + `useCampaignMetadata`; Info tab renders the JSON description. `BuyPanel` reads `getAcceptedTokens` + `tokenConfigs` + ERC20 symbol/decimals/balance/allowance, then orchestrates approve → buy. `StakingPanel` reads user positions via `getPositions` + batched `positions`/`earned` calls (10s refetch), shows live unstake penalty %, orchestrates approve → stake and per-position claim/unstake/restake tx flows. `HarvestPanel` lists seasons via `useCampaignSeasons` subgraph query, reads each user's `claims(seasonId, user)` on `HarvestManager`, and drives the two-step USDC flow (`redeemUSDC` → `claimUSDC`) with live pro-rata entitlement math (`owed × deposited / totalOwed − alreadyClaimed`). Product-redemption UI is wired to the Merkle backend: `ProductRedemption` subcomponent fetches `/api/merkle/:campaign/:seasonId/:user` via React Query, shows the allocation, and calls `HarvestManager.redeemProduct(seasonId, yieldAmount, proof)` when the user clicks. 404 → "not eligible" state.
