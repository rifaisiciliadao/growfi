...
- **Pages**:
  ...
  - `/campaign/[address]` — Invest / Stake / Harvest / Info tabs. Hero (title, image, location, breadcrumb) resolves from `useSubgraphCampaign` + `useCampaignMetadata`; Info tab renders the JSON description. `BuyPanel` reads `getAcceptedTokens` + `tokenConfigs` + ERC20 symbol/decimals/balance/allowance, then orchestrates approve → buy.
