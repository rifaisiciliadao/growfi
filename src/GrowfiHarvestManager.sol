...
/// @title GrowfiHarvestManager — Harvest Reporting & Two-Step Redemption
...
contract GrowfiHarvestManager is Initializable, ReentrancyGuard, PausableUpgradeable {
    ...
    /// @notice Owning GrowfiCampaign proxy. Set once by the factory; needed so the
    ///         GrowfiCampaign can deposit on behalf of holders out of its
    ...
    /// @notice Set the GrowfiYieldToken address. Can only be called once by the factory.
    ...
    /// @notice Set the GrowfiStakingVault address. Can only be called once by the factory.
    ...
    function setStakingVault(address stakingVault_) external onlyFactory {
        ...
        stakingVault = GrowfiStakingVault(stakingVault_);
        _stakingVaultSet = true;
    }

    /// @notice Set the owning GrowfiCampaign proxy. Called once by the factory after
    ///         the GrowfiCampaign proxy has been deployed. Required for
    ///         `depositFromCollateral` to be callable by the GrowfiCampaign.
    ...
    /// @notice Single deposit entry point — only the owning GrowfiCampaign may
    ///         call. Both producer-funded deposits (via `GrowfiCampaign.depositUSDC`,
    ///         pre-deadline) and post-deadline collateral settlements (via
    ///         `GrowfiCampaign.settleSeasonShortfall`) route through here. The
    ///         GrowfiCampaign side decides where the USDC came from (wallet vs
    ...
    /// @dev    `usdcDeadline` is enforced by `GrowfiCampaign.depositUSDC` (producer
    ///         path) and skipped on `GrowfiCampaign.settleSeasonShortfall` (post-
    ...
    ///         (= the owning GrowfiCampaign) and splits it 98/2 into the season
