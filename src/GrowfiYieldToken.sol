...
contract GrowfiYieldToken is Initializable, ERC20Upgradeable {
    ...
    /// @notice Mint yield tokens. Only callable by GrowfiStakingVault during staking.
    function mint(address to, uint256 amount) external onlyStakingVault {
        _mint(to, amount);
    }

    /// @notice Burn yield tokens. Callable by GrowfiStakingVault (forfeit) or GrowfiHarvestManager (redemption).
